...
error_reporting(1);
 session_start();
	...
	// Services
	$service = $pdo->prepare("SELECT * FROM subjects  ORDER BY subject ASC");
	$service->execute();
	...
	***/
		...
		$searchs = "SELECT * FROM papers WHERE grade = ?  ORDER BY timestamp DESC LIMIT 100";
		$stmt = $pdo->prepare($searchs);
		$stmt->execute([$theid]);
		$QP = $stmt->fetchAll();
		
		$delete = "DELETE  FROM papers WHERE name = ?";
